...
public class SalaDAO {
	...
	public static void deleteSala(Object object, int cuitEstablecimiento) throws SQLException {
		...
		Connection coneccion = PoolConnection.getPoolConnection().getConnection();
		PreparedStatement query = coneccion.prepareStatement(
				"update salas set activa = 0 where " + CUIT_ESTABLECIMIENTO + " = ? and nombre like ?");
		query.setInt(1, cuitEstablecimiento);
		query.setString(2, sala.getNombre());
		query.execute();
...
//			query.execute();
	...
	public static Map<FilaColumna, AsientoFisico> selectAsientosSala(Sala sala, int cuitEstablecimiento)
			throws SQLException {
		Connection coneccion = PoolConnection.getPoolConnection().getConnection();
		PreparedStatement query = coneccion
				.prepareStatement("select fila,columna,nro_fila,nro_columna,usable from sala_asientos where "
						+ CUIT_ESTABLECIMIENTO + " = ? and nombre_sala like ?");
		query.setInt(1, cuitEstablecimiento);
		query.setString(2, sala.getNombre());
		ResultSet rs = query.executeQuery();
		PoolConnection.getPoolConnection().realeaseConnection(coneccion);
		...
		while (rs.next()) {
			asientos.put(new FilaColumna(rs.getString(1), rs.getString(2)),
					new AsientoFisico(rs.getString(1), rs.getString(2), rs.getInt(3), rs.getInt(4), rs.getBoolean(5)));
		}
		return asientos;
	}
	...
	public static Sala selectSala(String nombre, Integer cuit) throws SQLException {
		Connection coneccion = PoolConnection.getPoolConnection().getConnection();
		PreparedStatement query = coneccion.prepareStatement(
				"select * from salas where " + CUIT_ESTABLECIMIENTO + " = ? and nombre = ? and activa = 1");
		query.setInt(1, cuit);
